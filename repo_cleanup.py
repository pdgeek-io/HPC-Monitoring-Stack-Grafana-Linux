...
# --- File mapping used in the repository (excluding README) ---
files = {
...
# --- Functions to compute required directories and cleanup old ones ---

def compute_required_dirs(files_dict):
    required = set()
    for file_path in files_dict:
        ...
        for i in range(1, len(parts)):
            ...
            required.add(dir_path)
    return required

def cleanup_old_dirs(required_dirs):
    # Walk through the repository from the bottom up.
    for root, dirs, _ in os.walk(".", topdown=False):
        # Skip the repository root itself and any hidden directories (like .git)
        if root == ".":
            continue
        rel_root = os.path.relpath(root, ".")
        if any(part.startswith('.') for part in rel_root.split(os.sep)):
            continue
        # If the current directory is not in the required set, remove it.
        if rel_root not in required_dirs:
            print(f"Removing old directory: {rel_root}")
            shutil.rmtree(root)

def main():
    required_dirs = compute_required_dirs(files)
    print("Required directories:")
    for d in sorted(required_dirs):
        print(f"  {d}")
    cleanup_old_dirs(required_dirs)
    print("Cleanup of old directories completed.")
